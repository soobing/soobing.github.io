...
클라이언트 상태와 서버 상태의 차이에 대해 알아보겠습니다. 이는 React Query가 어떤 역할을 하고 왜 또 다른 라이브러리가 서버 상태 관리를 위해 필요한지 이해하는 데 도움이 됩니다.
...
기존에는 React 개발자들은 클라이언트 상태 관리를 위해 Redux나 React Context API를 사용해왔습니다. API 데이터와 상호작용할 때에는 주로 `useState` 훅과 `useEffect` 훅의 조합을 통해 fetch API 데이터를 가져와 클라이언트 상태에 반영합니다.
...
useQuery 후크는 두 가지 필수 옵션(속성)을 사용합니다.
...
`queryFn` ****은 useQuery 훅이 호출될 때 React Query에 의해 실행되는 콜백 함수입니다. 이 함수는 useQuery 훅의 실행 중 특정 시점에서 명세한 작업(데이터 페칭)을 수행합니다.
...
이 두 가지 옵션 외에 선택적 속성을 가질 수도 있습니다. 다음 섹션에서 중요한 몇 가지를 다룰 예정 입니다.
...
`useQuery` 훅은 객체를 반환합니다. 이 객체는 쿼리의 상태에 대한 정보가 포함되어 있습니다. 중요한 속성 및 메서드 중 일부는 아래와 같습니다.

**속성**

1. `data`: 성공적으로 페칭된 경우 쿼리로 부터 반환된 데이터
2. `error`: 쿼리 도중 발생한 모든 에러(발생한 경우)
...
따라서, 이러한 속성들의 값에 접근하기 위해 JavaScript 구조 분해 할당을 사용할 수 있습니다.
...
| useQuery 옵션 | 역할 | 기본값 |
| --- | --- | --- |
| refetchOnWindowFocus | 데이터가 상한(stale) 경우 브라우저 창이 포커스 되면 리페치 | true |
| refetchOnMount | 데이터가 상한 경우 마운트 시 리페치 | true |
| refetchOnReconnect | 데이터가 상한 경우 재연결 시 리페치 | true |

브라우저를 백그라운드에서 유휴(idle) 상태로 유지하는 상황을 가정해보겠습니다. 누군가는 API 데이터/서버를 업데이트하기 위해 요청을 보내고 데이터가 업데이트됩니다. 그러나 브라우저는 백그라운드에 있기 때문에 UI가 업데이트되지 않습니다. useQuery에는 API 엔드포인트/서버에서 업데이트된 데이터를 다시 가져올 트리거가 없습니다. 이러한 상황에서는 `refetchInterval` 옵션을 사용할 수 있습니다. 이 옵션을 사용하면 지정된 밀리초 단위의 주기로 useQuery를 실행하여 데이터를 다시 가져올 수 있습니다. 이렇게 하면 브라우저 창이 유휴(idle) 상태일 때에도 데이터를 다시 가져올 수 있습니다.
...
`useMutation` 훅은 객체를 반환합니다. 이 객체에는 변이와 상호 작용하고 결과를 처리하는 데 사용할 수 있는 여러 속성과 메서드가 포함되어 있습니다. 그 중 일부는 다음과 같습니다.
...
useQuery 훅을 사용하여 원격 서버에서 일부 사용자 데이터를 가져오는 경우가 있다고 가정해 봅시다. 데이터를 받는 동안 시간이 걸릴 수 있습니다. 작업을 빠르게 하기 위해, React Query는 이 사용자 데이터를 캐시에 저장하여 후속 요청시 빠르게 찾을 수 있도록 합니다. 이를 통해 데이터 로드하는 데 걸리는 시간을 줄일 수 있습니다.
...
기본적으로, useQuery는 이 캐시 데이터를 상한(stale) 상태로 표시합니다. React Query 캐싱과 관련된 두 가지 옵션을 알아야 합니다. 바로 `staleTime`과 `cacheTime`입니다.

`staleTime`: 쿼리 결과가 상한 것으로 간주되는 데 걸리는 시간을 결정합니다. 시간은 밀리초로 지정됩니다.
...
이전에 설명한 대로, 캐시에 저장된 데이터는 staleTime 옵션에 따라 유효하지 않아질 수 있습니다. 그러나 `staleTime`을 재정의하고 무효(invalid)하거나 오래(stale)되었다고 표시해야 하는 상황이 발생할 수 있습니다. 예를 들어, API에 POST 요청을 보낼 때는 API 엔드포인트의 데이터가 최신 데이터이므로 수동으로 캐시에 있는 데이터를 유효하지 않은 것으로 표시해야 합니다. 결과적으로, POST 요청 직후에는 캐시의 데이터가 오래되었다고 간주됩니다.
...
[React Query 에서 쿼리들](https://tanstack.com/query/v4/docs/react/guides/queries)
